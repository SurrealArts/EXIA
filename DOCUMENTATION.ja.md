...
5. [モジュール](#5-モジュール)
   ...
   - 5.6 メンションガード (`src/modules/mentionGuard.js`)
6. [コマンド](#6-コマンド)
...
7. [ユーティリティ](#7-ユーティリティ)
   ...
   - 7.3 クエリキャッシュ (`src/utils/queryCache.js`)
8. [通信フロー](#8-通信フロー)
...
│   │   ├── locale.test.js            # ロケールの Vitest スイート
...
│   │   ├── mentionGuard.js           # @everyone/@here/@role 検出 + 動的乗数
│   │   ├── mentionGuard.test.js      # メンションガードの Vitest スイート
│   │   ├── raidProtection.js         # 3 段階レイドエスカレーション、権限バックアップ/復元
│   │   ├── raidProtection.test.js    # レイドプロテクションの Vitest スイート
...
│   │   ├── regexWorker.test.js       # 正規表現ワーカーの Vitest スイート
...
│       ├── queryCache.js             # TTL ベースのクエリキャッシュ (LRU 削除)
│       ├── queryCache.test.js        # クエリキャッシュの Vitest スイート
...
| `pino`           | ^10.3.1    | 構造化 JSON ロガー (v1.1.0 で削除)                        |
...
- **5 モジュール:** user_profile (15)、velocity (10)、honeypot (0, critical)、regex (10)、mention_guard (10)
...
サーバーごとの言語設定を提供する i18n システム。ロケールファイルは `src/locales/` に JSON キー値マップとして保存されます。

#### 主要エクスポート

| エクスポート               | 説明                                                                 |
| -------------------------- | -------------------------------------------------------------------- |
| `t(lang, key, vars?)`      | 指定された言語のキーを翻訳、オプションで変数を補間                   |
| `getGuildLanguage(db, id)` | サーバーに設定された言語を取得（デフォルト `"en"`）                  |
| `resolveInteractionLang()` | インタラクションから言語を解決（サーバー設定またはユーザーロケール） |
...
3. user_profile 監査 → 乗数計算
  ↓
4. メンションガードチェック (fast-track でない場合) → @everyone/@here/@role 検出、動的乗数
  ↓
5. ベロシティチェック (fast-track でない場合) → トークン消費、マルチチャンネル検出
  ↓
6. 正規表現評価 (fast-track またはステッカーのみでない場合) → ルールごとにワーカースレッド
  ↓
7. プロフィール理由収集 → flagReasons に追加
  ↓
8. いずれかのモジュールがアクションをトリガーした場合、制裁実行
  ↓
9. すべてのフラグ理由に対してテレメトリエンキュー
```
...
`checkMentionGuard(message, lang)` (§5.6 参照) を呼び出す。メッセージに @everyone、@here、またはロールメンションが含まれ、ユーザーがメッセージ管理権限を持っていない場合：

- 重み = `module_weight × mention_multiplier` (mention_multiplier: 初回 3.0x、繰り返し +1.0x、上限 5.0x)
- `mention_multiplier` はクリーンなメッセージごとに `cleanStreak × 0.1` だけ減衰
- `applyPressure` を有効な重みで呼び出す
- プロフィール乗数はメンション乗数と複合しない（独立した軸）
...
- モジュールフィールド: モジュールごとに 3 列 (絵文字+名前、重み、クリティカル)
  ...
  - `mention_guard` は有効圧力範囲 (重み × 3.0~5.0) を重みとして表示
...
- モジュールテーブル: 名前、重み/クリティカル/有効 (regex は重み範囲、user_profile は段階、mention_guard は有効圧力範囲を表示)
...
@everyone、@here、ロールメンションをモデレーター以外のユーザーから検出します。メッセージ管理権限または管理者権限を持つユーザーは除外されます。
...
1. **権限チェック:** ユーザーが `ManageMessages` または `Administrator` を持っている場合 → 除外
2. **メンション検出:** `message.mentions.everyone`、`message.mentions.here`、`message.mentions.roles` を確認
3. **トリガーされた場合** (メンションあり + 除外なし):
   - 初回: `multiplier = 3.0` を設定
   - 繰り返し: `multiplier += 1.0` (上限 5.0)
   - `{ triggered: true, multiplier, reasons }` を返す
4. **トリガーされなかった場合** でアクティブな状態がある場合:
   - `cleanStreak` を増加
   - `multiplier -= cleanStreak × 0.1` で減衰 (1.0 未満にならないようにクランプ)
   - multiplier ≤ 1.0 になったらエントリを削除
...
| ----------------------------- | ---------- | ------------------ |
| 初回メンション                | 3.0x       | 30p                |
| 2回目のメンション (クリーンなし) | 4.0x       | 40p                |
| 3回目以降 (クリーンなし)      | 5.0x (上限)| 50p                |
| 3回のクリーンメッセージ後     | ~2.4x      | 24p (T1 未満)      |

乗数は **独立** しています — profile_mult は mention_mult と複合しません。

#### 定数

| 定数                       | 値   | 説明                                  |
| -------------------------- | ---- | ------------------------------------- |
| `MENTION_BASE_MULTIPLIER`  | 3.0  | 初回メンション時の開始乗数            |
| `MAX_MENTION_MULTIPLIER`   | 5.0  | 繰り返し違反者の上限                  |
| `CLEAN_DECAY_PER_MESSAGE`  | 0.1  | クリーンメッセージあたりの基本減衰量、ストリークでスケーリング |
...
モジュール選択肢: `user_profile`、`velocity`、`honeypot`、`regex`、`mention_guard`
...
スラッシュコマンドビルダー: `/debug` (Administrator 権限が必要)。
...
言語選択肢: `en` (英語)、`ja` (日本語)。サーバーの言語設定をデータベースに保存します。以降のすべての埋め込みとテレメトリは選択された言語を使用します。
...
TTL ベースのインメモリクエリキャッシュ。ホットパスでの不要な SQLite クエリを削減します。

| エクスポート        | 説明                                                   |
| ------------------- | ------------------------------------------------------ |
| `cachedGet(db, fn)` | `fn(db)` を実行し、関数 ID + DB 状態で結果をキャッシュ |
| `cachedAll(db, fn)` | `cachedGet` と同じだが複数行結果用                     |
| `invalidateCache()` | すべてのキャッシュエントリをクリア                     |

- **最大エントリ数:** 10,000（最も古いものから `evictIfNeeded()` で削除）
- **TTL:** エントリごとに 5 分
- 使用箇所: `messageCreate.js`（モジュール重み、しきい値）、`interactionCreate.js`（ギルド設定）、`guildMemberAdd.js`（モジュール重みチェック）
...
ボットは `ClientReady` 時にグローバルに 6 つのスラッシュコマンドを登録します。SQLite データは `exia_data` 名前付きボリュームに永続化されます。
...
- `mentionGuard.test.js` — @everyone/@here/@role トリガー、除外、エスカレーション、減衰、上限
- `telemetryQueue.test.js` — エンキュー、フラッシュ、チャンク分割、フィールド制限
- `locale.test.js` — 翻訳、フォールバック、補間、ギルド言語解決
- `queryCache.test.js` — キャッシング、削除、無効化
- `raidProtection.test.js` — ステージ遷移、自動検出、権限バックアップ/復元、スパイクウィンドウ
